...
def index():
    ...
    portfolio = db.execute("SELECT symbol, SUM(shares) as shares, price FROM transactions WHERE user_id = :user_id GROUP BY symbol", user_id=session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        shares = int(request.form.get("shares"))# converts shares to an int value (will need later to calculate cost)
        stock = lookup(request.form.get("symbol"))
        if not stock:
            return apology("Invalid symbol.")
        elif not request.form.get("shares") or shares < 0:
            return apology("Invalid number of shares.")
        ## select (one row one column input) the cash value from user in the database
        rows = db.execute("SELECT cash FROM users WHERE id = :user_id", user_id=session["user_id"])
        ...
        if total_cash < total_cost:
            return apology("not enough dough")
...
def quote():
    ...
    if request.method == "POST":
        ...
        return render_template("showQuote.html", stock=stock)
    else:
...
def register():
    ...
    if request.method == "POST":
        ...
        # ensure confirmed password is filled
        ...
        #check username is unique and add to database
        # execute will fail if username is not unique, so we can insert into the db
        result = db.execute("INSERT INTO users (username, hash) VALUES (:username, :hash)",
                username=request.form.get("username"), hash=hash)
        # if username exists already... i.e if db.execute fails
        if not result:
